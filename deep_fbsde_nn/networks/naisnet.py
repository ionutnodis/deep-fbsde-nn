...
NAIS-Net enforces stability via spectral projection,
ensuring each residual block is a contraction mapping.

Projection mechanism:
    Given weight matrix W, compute:
    1. R^T R = W^T W
    2. If ||R^T R|| > δ, rescale: R^T R ← √δ · R^T R / √||R^T R||
    3. A = R^T R + ε·I
    4. Output: h' = -A @ h + b + skip(input)

This guarantees ||∂h'/∂h|| < 1, making each block contractive.
...
class NAISNet(nn.Module):
    ...
    def _project(self, W: torch.Tensor) -> torch.Tensor:
        ...
        Computes A = clip(W^T W, δ) + εI where clip rescales if norm > δ.
        """
        WtW = W.t() @ W
        norm = torch.norm(WtW)
        
        if norm > self.delta:
            WtW = WtW * (self.delta / norm).sqrt()
